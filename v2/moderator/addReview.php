        ...
        <div class="bg-white border border-gray-100 shadow-md shadow-black/5 p-6 rounded-md lg:col-span-3">
            ...
            <div>
                ...
                <form action="addReview.php" method="post">
                    ...
                    <div class="mb-6">
                        <label for="org" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Organisation Name</label>
                        <select name="org" id="org" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required>
                            <option value=''>--SELECT--</option>
                            <?php 
                            $sql = "SELECT current_org FROM `admin`";
                            $result = mysqli_query($conn, $sql);
                            $sno = 0;
                            while ($row = mysqli_fetch_assoc($result)) {
                                echo "<option value='".$row['current_org']."'>".$row['current_org']."</option>";
                            }
                            ?>
                        </select>
                    </div>
